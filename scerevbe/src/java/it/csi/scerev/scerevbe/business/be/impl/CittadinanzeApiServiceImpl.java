...
import it.csi.scerev.scerevbe.business.be.CittadinanzeApi;
...
public class CittadinanzeApiServiceImpl implements CittadinanzeApi {
	//getcittadinanze
	public Response cittadinanzeGet(
		...
		final String operazione = it.csi.scerev.scerevutil.business.SharedConstants.OPERAZIONE_GET;
		...
		ServiziEsterniClient.getInstance();
		try {						
			...
			return Response.status(Status.OK).entity(listCittadinanze).header(Constants.X_REQUEST_ID, ""+xRequestId).build();
		} catch (final Exception e) {
			e.printStackTrace();
			MessaggioFacadeIf messaggioFacade = (MessaggioFacadeIf) SpringApplicationContextHelper.getBean(SharedConstants.MESSAGGIO_FACADE);
			ModelMessaggio msg = messaggioFacade.getMessaggioPerCodice(Constants.ERROR_500_GETLISTACITTADINANZE);
