...
import it.csi.scerev.scerevbe.business.be.MotivazioniDomicilioApi;
...
public class MotivazioniDomicilioApiServiceImpl implements MotivazioniDomicilioApi {
	...
	public Response motivazioniDomicilioGet(
		...
		try {
			...
			List<ModelMotivazioneDomicilio> listMotivazioniDomicilio = listeFacade.getMotivazioniDomicilio();
			...
			return Response.status(Status.OK).entity(listMotivazioniDomicilio).header(Constants.X_REQUEST_ID, ""+xRequestId).build();
		} catch (final Exception e) {
			e.printStackTrace();
			MessaggioFacadeIf messaggioFacade = (MessaggioFacadeIf) SpringApplicationContextHelper.getBean(SharedConstants.MESSAGGIO_FACADE);
			ModelMessaggio msg = messaggioFacade.getMessaggioPerCodice(Constants.ERROR_500_GETLISTAMOTIVAZIONIDOMICILIO);
